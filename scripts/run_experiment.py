#!/usr/bin/env python
"""End-to-end annealing experiment: sample → train → anneal → plot.

Edit the USER CONFIGURATION block at the top, then run:
    uv run python scripts/run_experiment.py

To re-plot from already-saved results without re-running the pipeline:
    uv run python scripts/run_experiment.py --plot-only
"""
...
# -- import sibling scripts so we can reuse their run_config functions --
...
BETA_H      = 10.0
INIT_SOURCE = "model"   # "mcmc" or "model"
...
# Annealing — ULA proposal
N_ULA_STEPS   = 10
ULA_STEP_SIZE = 1e-3

# Annealing — diffusion proposal (mutation kernel)
N_DIFFUSION_STEPS  = 10
DIFF_T_START       = 0.05
DIFF_T_END         = 1e-3
DIFF_SCORE_SCALING = True   # scale score by beta_curr/beta_train

# Initial particles when INIT_SOURCE="model"
INIT_N_DIFFUSION_STEPS = 500
...
# Short label appended to the experiment directory name — change to distinguish runs.
# Leave as "" to omit.
DESC = ""
# ===========================================================================
...
def _experiment_dir() -> Path:
    tag = f"{ENERGY}_d{DIM}_betaH{_beta_str(BETA_H)}_{INIT_SOURCE}"
    if DESC:
        tag += f"_{DESC}"
...
# ---------------------------------------------------------------------------
# Config factories (match the conventions of create_sampler/training_configs.py)
# ---------------------------------------------------------------------------
...
def run_all_annealing(
    model_runs: dict[float, str],
    device: torch.device,
) -> list[dict]:
    """Run both SMC methods for every model. Returns list of result dicts sorted by beta_m."""
    results = []
    for beta_m in sorted(model_runs):
        print(f"\n  beta_M={beta_m}  [{model_runs[beta_m]}]")
        result = _run_annealing(model_runs[beta_m], device)
...
def _get_initial_particles(
    rsde, sample_run: str, device: torch.device,
) -> torch.Tensor:
    if INIT_SOURCE == "mcmc":
        pts = torch.load(
        ...
        x = pts[:, -1, :]  # last MCMC snapshot: [N, dim]
        if x.shape[0] > N_PARTICLES:
            x = x[torch.randperm(x.shape[0], device=device)[:N_PARTICLES]]
        return x
    elif INIT_SOURCE == "model":
        sampler = DiffusionModelSampler(rsde, n_steps=INIT_N_DIFFUSION_STEPS)
        return sampler.sample(N_PARTICLES, device, show_progress=True)
    else:
        raise ValueError(f"Unknown INIT_SOURCE: {INIT_SOURCE!r}")
...
def _run_annealing(model_run: str, device: torch.device) -> dict:
    rsde, energy, dim, beta_m, sample_run = _load_model(model_run, device)
    x0            = _get_initial_particles(rsde, sample_run, device)
    ...
    # SMCSampler.run() clones initial_cloud internally, so both methods get the same start
    ...
    ula_cloud, ula_diag = ula_sampler.run(initial_cloud, beta_ladder, show_progress=True)

    print("    Diffusion SMC...")
    diff_proposal = DiffusionAnnealingProposal(
        rsde, energy, beta_train=beta_m,
        n_diffusion_steps=N_DIFFUSION_STEPS,
        t_start=DIFF_T_START,
        t_end=DIFF_T_END,
        score_scaling=DIFF_SCORE_SCALING,
    )
    diff_sampler  = SMCSampler(
        diff_proposal.mutation_kernel, diff_proposal.weight_update, energy,
        ess_threshold=ESS_THRESHOLD,
    )
    diff_cloud, diff_diag = diff_sampler.run(initial_cloud, beta_ladder, show_progress=True)
...
def _print_result(r: dict) -> None:
    u, d = r["ula"], r["diffusion"]
    print(f"    ULA:       mean={u['mean_energy']:.4f}  min={u['min_energy']:.4f}  ESS={u['final_ess']:.3f}")
    print(f"    Diffusion: mean={d['mean_energy']:.4f}  min={d['min_energy']:.4f}  ESS={d['final_ess']:.3f}")
...
def _config_snapshot() -> dict:
    return {
        ...
        "annealing": {
            ...
            "init_model": {
                "n_diffusion_steps": INIT_N_DIFFUSION_STEPS,
            },
        },
    }
...
def plot_results(path: Path | None = None) -> None:
    ...
    diff_mean = [r["diffusion"]["mean_energy"] for r in runs]
    ula_min   = [r["ula"]["min_energy"]      for r in runs]
    diff_min  = [r["diffusion"]["min_energy"]  for r in runs]

    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(10, 4), sharey=False)

    for ax, ula_vals, diff_vals, ylabel in [
        (ax1, ula_mean, diff_mean, "Mean energy"),
        (ax2, ula_min,  diff_min,  "Min energy"),
    ]:
        ax.plot(beta_ms, ula_vals,  "o-", label="ULA SMC",       color="steelblue")
        ax.plot(beta_ms, diff_vals, "s-", label="Diffusion SMC", color="darkorange")
    ...
    diff_cfg = cfg["annealing"]["diffusion"]
    subtitle = (
        f"{cfg['energy']}  d={cfg['dim']}  init={cfg['init_source']}{desc_str}\n"
        f"diff: t_start={diff_cfg['t_start']}  n_steps={diff_cfg['n_steps']}  "
        f"score_scaling={diff_cfg['score_scaling']}  |  "
        f"ula: step={cfg['annealing']['ula']['step_size']}  n_steps={cfg['annealing']['ula']['n_steps']}"
    )
...
def main() -> None:
    ...
    print(f"=== {ENERGY}  d={DIM}  beta_H={BETA_H}  init={INIT_SOURCE} ===")
    ...
    print("\n[3/3] Annealing")
    results = run_all_annealing(model_runs, device)

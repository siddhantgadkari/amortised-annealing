...
def _make_config(run_name: str, preset_name: str, preset: dict, seed: int) -> dict:
    model_run_name = f"{run_name}_{_model_tag(preset_name)}_seed{seed}"
    return {
        "job": {
            "seed":   seed,
            "device": "auto",
...
def main() -> None:
    ...
    count = 0
    for run_name, (preset_name, preset), seed in product(sample_runs, presets.items(), seeds):
        cfg = _make_config(run_name, preset_name, preset, seed)

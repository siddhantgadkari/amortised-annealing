#!/usr/bin/env python
"""End-to-end sweep for Rastrigin: sample → train → eval.
...
from amortised_annealing.energies import Rastrigin
...
def run_sampling(dim: int, beta: float, device: torch.device) -> str:
    run_name = f"rastrigin_d{dim}_beta{_beta_str(beta)}_{MCMC_METHOD.lower()}_seed{SEED}"
    ...
    cfg = {
        ...
        "target":  {"energy": "rastrigin", "dim": dim, "beta_m": beta},
    ...
    summary = {
        "status": "completed", "energy": "rastrigin", "dim": dim, "beta_m": beta,
...
def main() -> None:
    ...
    for dim in DIMS:
        for beta in BETAS:
            print(f"\n{'─'*60}")
            print(f"  rastrigin  dim={dim}  beta={beta}")
            print(f"{'─'*60}")
    ...
    out_path = ROOT / "data" / "rastrigin_sweep_results.json"
